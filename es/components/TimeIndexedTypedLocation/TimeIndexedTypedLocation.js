...
var D3_ZINDEX = 625; // over markers under tooltip and popup
...
function TimeIndexedTypedLocation(_ref) {
  var timeIndexedTypedLocations = _ref.timeIndexedTypedLocations,
      cPropDepiction = _ref.cPropDepiction,
      cPropLabel = _ref.cPropLabel,
      _ref$onCulturalProper = _ref.onCulturalPropertyClick,
      onCulturalPropertyClick = _ref$onCulturalProper === void 0 ? function () {} : _ref$onCulturalProper;
  ...
  (0, _ldUiHooks.usePane)(mapRef, "d3-layer", D3_ZINDEX);
  /**
  ...
  (0, _react.useEffect)(function () {
    ...
    var geometries = true;
    timeIndexedTypedLocations.forEach(function (tITL, index) {
    ...
    if (geometries) {
      var adaptD3Layer = function adaptD3Layer() {
      ...
      var moveLine = function moveLine() {
      ...
      timeIndexedTypedLocations.forEach(function (tITL, index) {
        ...
        var popupContent = {
          ...
          locationType: tITL.locationType.split("/").pop(),
          // at the moment we pass the uri TODO: pass the label
          culturalProperty: tITL.cPropLabel !== "" ? tITL.cPropLabel : "Bene culturale",
        ...
        }).addTo(mcg).bindPopup(popup).on("click", function (e) {
          this.openPopup();
          var cProp = document.getElementsByClassName("cultural-property");
          console.log(cProp);
      ...
      var arrowheads = g.selectAll(".arrowheads").data(geoJSON.features).enter().append("svg:path").attr("class", "arrowheads").attr("d", d3.symbol().type(d3.symbolTriangle).size(_config["default"].ARROW.ARROWHEAD_SIZE)).style("stroke", "blue").style("fill", "blue"); // check if the depiction has been put between timeIndexTypedLocation pattern data

      cPropDepiction = cPropDepiction ? cPropDepiction : timeIndexedTypedLocations[0].depiction;
      var depiction = g.append("svg:image").attr("x", -20).attr("y", -20); // .attr("height", 120)
      // .attr("width", 60)
