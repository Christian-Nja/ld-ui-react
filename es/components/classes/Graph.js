...
var Graph = /*#__PURE__*/function () {
   ...
   * @memberof Graph
   */
  function Graph(nodes, edges, filters, palette) {
  ...
  _createClass(Graph, [{
    ...
    value: function addFilter(filter) {
      var filterIdx = this.findFilterIdx(filter.key);
      filterIdx !== -1 ? this.filters[filterIdx] = filter : this.filters.push(filter);
    }
  }, {
    key: "removeFilter",
    value: function removeFilter(filterKey) {
      this.filters.filter(function (entry) {
        return entry.key !== filterKey;
      });
    }
  }, {
    key: "findFilterIdx",
    value: function findFilterIdx(filterKey) {
      return this.filters.findIndex(function (activeFilter) {
        return activeFilter.key === filterKey;
      });
    }
  }, {
    key: "applyFilter",
    value: function applyFilter(nodes, filter) {
      return nodes.filter(function (node) {
        if (filter["class"] === "cat") {
          return filter.mask.find(function (nodeToFilterOut) {
            return node.data[filter.key] === nodeToFilterOut;
          }) === undefined;
        } else if (filter["class"] === "num") {
          return node.data[filter.key] >= filter.range[0] && node.data[filter.key] <= filter.range[1];
        } else if (filter["class"] === "time") {
          console.log(filter.mask);
          console.log(filter.key);
          return filter.mask.find(function (nodeToKeep) {
            return node.data[filter.key] === nodeToKeep;
          }) !== undefined;
        } else if (filter["class"] === "search") {
          return node.data[filter.key].toLowerCase().includes(filter.mask.toLowerCase());
        }
      });
    }
  }, {
    ...
    value: function filteredNodes() {
      ...
      this.filters.forEach(function (filter) {
        filtered = _this3.applyFilter(filtered, filter);
      });
      return filtered;
    }
  }, {
    key: "filteredEdges",
    value: function filteredEdges(nodes) {
      var nodeIds = nodes.map(function (node) {
        return node.id;
      });
      return this.edges.filter(function (edge) {
        return nodeIds.includes(edge.target) && nodeIds.includes(edge.source);
      });
    }
  }, {
    key: "toVisual",
    value: function toVisual() {
      var filtered = this.filteredNodes();
      return {
        nodes: filtered.map(function (node) {
          return node.toJson();
        }),
        edges: this.filteredEdges(filtered).map(function (edge) {
          return edge.toJson();
        })
      };
    }
    ...
    value: function getAdjacents(node) {
      ...
      this.edges.forEach(function (edge) {
        if (edge.source === node.id) {
          adjacents.push(_this4.getNodeById(edge.target));
        } else if (edge.target === node.id) {
          adjacents.push(_this4.getNodeById(edge.source));
        }
      });
      return adjacents;
    }
          ...
          if (!visited.includes(node.id)) {
            (function () {
              ...
              while (!queue.isEmpty()) {
                ...
                var adjacents = _this5.getAdjacents(nextNode);
