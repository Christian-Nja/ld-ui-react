...
function passOver(p1, p2) {
  var tolerance = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

  var x_range = _underscore["default"].range(parseInt(p2[X]) - tolerance, parseInt(p2[X]) + tolerance);

  var y_range = _underscore["default"].range(parseInt(p2[Y]) - tolerance, parseInt(p2[Y]) + tolerance);

  if (x_range.includes(parseInt(p1[X])) && y_range.includes(parseInt(p1[Y]))) {
    return true;
  } else {
    return false;
  }
